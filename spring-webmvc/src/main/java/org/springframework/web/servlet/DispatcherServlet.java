...
public class DispatcherServlet extends FrameworkServlet {
	...
	/**
	 * Request attribute 中保存的 WebApplicationContext 的key
	 */
	public static final String WEB_APPLICATION_CONTEXT_ATTRIBUTE = DispatcherServlet.class.getName() + ".CONTEXT";

	/**
	 * Request attribute 中保存的 环境解析器(LocaleResolver) 的key
	 */
	public static final String LOCALE_RESOLVER_ATTRIBUTE = DispatcherServlet.class.getName() + ".LOCALE_RESOLVER";

	/**
	 * Request attribute 中保存的 主题解析器(ThemeResolver) 的key
	 */
	public static final String THEME_RESOLVER_ATTRIBUTE = DispatcherServlet.class.getName() + ".THEME_RESOLVER";

	/**
	 * Request attribute 中保存的 主题(ThemeSource) 的key
	 */
	public static final String THEME_SOURCE_ATTRIBUTE = DispatcherServlet.class.getName() + ".THEME_SOURCE";

	/**
	 * Request attribute 中保存的 源请求的FlashMap 的key
	 * <p>比如说设置了FlashMap然后重定向后，那么就会保存源请求的FlashMap</p>
	 */
	...
	/**
	 * Request attribute 中保存的 FlashMapManager 的key
	 */
	public static final String FLASH_MAP_MANAGER_ATTRIBUTE = DispatcherServlet.class.getName() + ".FLASH_MAP_MANAGER";

	/**
	 * Request attribute 中保存的 异常 的key
	 * <p>
	 *     这里的异常是指在执行doDispatch方法过程中抛出的异常
	 * </p>
	 */
	public static final String EXCEPTION_ATTRIBUTE = DispatcherServlet.class.getName() + ".EXCEPTION";

	/** 当没有为请求找到映射的处理程序时使用的日志名称 */
	public static final String PAGE_NOT_FOUND_LOG_CATEGORY = "org.springframework.web.servlet.PageNotFound";

	/**
	 * DispatcherServlet的配置类文件的名称
	 */
	private static final String DEFAULT_STRATEGIES_PATH = "DispatcherServlet.properties";

	/**
	 * 有关 DispatcherServlet 的默认属性的开头前缀
	 */
	private static final String DEFAULT_STRATEGIES_PREFIX = "org.springframework.web.servlet";

	/** 当没有为请求找到映射的处理程序时使用的日志记录器 */
	protected static final Log pageNotFoundLogger = LogFactory.getLog(PAGE_NOT_FOUND_LOG_CATEGORY);

	/**
	 * 从配置文件中记载有关 {@link DispatcherServlet} 中八大组件的默认配置，默认没有 {@link RequestToViewNameTranslator}
	 * <p>比如说 {@link HandlerMapping} 默认有用逗号分隔的多个</p>
	 */
	private static final Properties defaultStrategies;

	static {
		// 从配置文件中记载有关 DispatcherServlet 中九大组件的默认配置
		try {
	...
	/**
	 * 是需要容器中所有 HandlerMappings 还是按照名称查找一个
	 * <p<>下面三个和这个是一样的作用</>
	 * */
	...
	/**
	 * 初始化此 {@link javax.servlet.Servlet} 使用的九大对象
	...
	/**
	 * 初始化 {@link MultipartResolver}
	 * <p>如果在 {@link org.springframework.beans.factory.BeanFactory} 中没有改Bean，则不提供多部分处理</p>
	 */
	...
	/**
	 * 初始化 {@link LocaleResolver}
	 * <p>如果在 {@link org.springframework.beans.factory.BeanFactory} 中没有改Bean，则使用默认的</p>
	 */
	...
	/**
	 * 初始化 {@link HandlerExceptionResolver}
	 * <p>如果在 {@link org.springframework.beans.factory.BeanFactory} 中没有改Bean，则使用默认的</p>
	 */
	...
	/**
	 * 初始化 {@link RequestToViewNameTranslator}
	 * <p>如果在 {@link org.springframework.beans.factory.BeanFactory} 中没有改Bean，则使用默认的</p>
	 */
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 调用处理器，并获得 ModelAndView 对象
				// 可能 ModelAndView 为空，比如说使用了@Response注解
				mv = ha.handle(processedRequest, response, mappedHandler.getHandler());

				// 判断异步任务是否已经启动
				// 如果是返回值是异步任务，那么是在对于的返回值处理器就已经开始在异步线程中执行具体的代码了
				if (asyncManager.isConcurrentHandlingStarted()) {
					return;
				}

				// 如果视图名为空，通过视图名转换器设置视图名称
				...
				// 从4.3开始，也在处理从处理程序方法抛出的错误，使它们可用于ExceptionHandler方法和其他场景
	...
	/**
	 * 处理 选择处理程序 和 处理程序执行的结果，该结果要么是ModelAndView，要么是要解析为ModelAndView的异常
	 * @param request
	 * @param response
	 * @param mappedHandler 处理器执行链
	 * @param mv
	 * @param exception 处理 选择处理程序 和 处理程序执行的结果之间的异常
	 * @throws Exception
	 */
	...
	/**
	 * 通过注册的异常处理器(HandlerExceptionResolvers)来处理派发过程中的异常和错误
	 * @param request current HTTP request
	 * @param response current HTTP response
